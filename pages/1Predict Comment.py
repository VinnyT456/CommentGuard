...
class YoutubeApp:
    ...
    def grab_video_comment(self):
        st.title("Video Comment Analyzer")
        url = st.text_input("Enter the video url")
        if url: 
            try:
               video_comments = [c['text'] for i,c in enumerate(self.downloader.get_comments_from_url(url, sort_by=1))]
            except:
               return
            
            video_comments = pd.DataFrame(video_comments,columns=["Comment"])
            processed_video_comments = video_comments["Comment"].apply(self.preprocess_comment)
            vectorized_video_comments = self.tfidf.transform(processed_video_comments)
            predictions = self.model.predict(vectorized_video_comments)

            video_comments['Prediction'] = predictions
            video_comments['Prediction'] = video_comments['Prediction'].apply(lambda x: "Spam" if x == 1 else "Not Spam")

            st.dataframe(video_comments)
    ...
    def run(self):
        ...
        if ('video_comment_prediction' not in st.session_state):
            st.session_state.video_comment_prediction = False
        ...
        if (st.sidebar.button('Video Comment Prediction')):
            st.session_state.single_comment_prediction = False
            st.session_state.multiple_comment_prediction = False
            st.session_state.video_comment_prediction = True
        ...
        if (st.session_state.video_comment_prediction):
            self.grab_video_comment()
